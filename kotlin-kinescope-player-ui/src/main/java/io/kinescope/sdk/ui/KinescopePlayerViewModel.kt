...
private const val KEY_AUDIO_TRACK_ID = "audio_track_id"
...
class KinescopePlayerViewModel(
    ...
    fun savedAudioTrackId(): Int? = savedStateHandle.get(KEY_AUDIO_TRACK_ID)

    fun persistTrackSelection(qualityId: Int, subtitleId: String, audioTrackId: Int) {
        savedStateHandle[KEY_QUALITY_ID] = qualityId
        savedStateHandle[KEY_SUBTITLE_ID] = subtitleId
        savedStateHandle[KEY_AUDIO_TRACK_ID] = audioTrackId
    }

    fun restoreTrackSelection(controller: KinescopeComposePlayerController) {
        savedQualityId()?.let(controller::setQuality)
        savedSubtitleId()?.let(controller::selectSubtitle)
        savedAudioTrackId()?.let(controller::selectAudioTrack)
    }
