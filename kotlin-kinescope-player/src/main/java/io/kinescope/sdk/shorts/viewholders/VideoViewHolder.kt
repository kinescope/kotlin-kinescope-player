...
import io.kinescope.sdk.databinding.ListVideoBinding
...
class VideoViewHolder(
    ...
    init {
        ...
        binding.btnOffline.setOnClickListener {
            videoData?.let { data ->
                // Ensure download subsystem is initialized and avoid "start download" toast
                ...
                if (!data.drm?.widevine?.licenseUrl.isNullOrBlank()) {
                    ...
                    playerManager.exposedDrmHelper.callback = { videoData, pssh ->
                        Log.d("DRM_DEBUG", "Callback вызван, PSSH размер: ${pssh.size}")
                        ...
                        downloadLicense(videoData, pssh) { keySetId ->
                            if (keySetId != null) {
                                ...
                                // Восстанавливаем позицию воспроизведения, если видео все еще воспроизводится
                    ...
                    // Проверяем все возможные источники PSSH БЕЗ сброса
                    var availablePssh: ByteArray? = null

                    // 1. Проверяем PSSH из DrmHelper (если уже был получен ранее)
                    availablePssh = playerManager.exposedDrmHelper.psshData

                    // 2. Проверяем текущий формат видео
                    ...
                    // 3. Проверяем треки плеера (currentTracks)
                    if (availablePssh == null) {
                        try {
                            ...
                            Log.d("DRM_DEBUG", "Ошибка при получении PSSH из треков: ${e.message}")
                        }
                    }

                    // Создаем локальную копию для безопасного использования в замыкании
                    val finalPssh = availablePssh
                    if (finalPssh != null && finalPssh.isNotEmpty()) {
                        Log.d("DRM_DEBUG", "PSSH найден сразу, размер: ${finalPssh.size}")
                        // Сохраняем PSSH в DrmHelper для будущего использования
                        playerManager.exposedDrmHelper.psshData = finalPssh
                        playerManager.exposedDrmHelper.callback?.invoke(data, finalPssh)
                    } else {
                        Log.d("DRM_DEBUG", "PSSH не найден, перезапускаем воспроизведение")
                        // Только теперь сбрасываем, если PSSH не найден
    ...
    fun startOfflineDownload(videoData: VideoData, keySetId: ByteArray) {
        if (videoData.hlsLink.isNullOrEmpty()) {
        ...
        try {
            ...
            val cleanVideoData = videoData

            val videoDataJson = try {
                AppJson.encodeToString(VideoData.serializer(), cleanVideoData)
            } catch (e: Exception) {
                Log.e("SERIALIZATION", "Error serializing VideoData", e)
                "{}"
            }

            Log.d("DOWNLOAD", "Saving VideoData with title: '${cleanVideoData.title}'")
            ...
            // Do not show on-video progress (it was centered and overlays content).
            binding.downloadProgress.visibility = android.view.View.GONE
        } catch (e: Exception) {
            Log.e("OFFLINE_DOWNLOAD", "Ошибка начала загрузки", e)
        }
    }
    private fun startClearVideoDownload(videoData: VideoData) {
        ...
        try {
            ...
            Log.e("OFFLINE_DOWNLOAD", "Ошибка при старте загрузки", e)
        }
    }
    ...
    private fun showOfflineVideosPopup() {
        ...
        adapter.setOnItemClicked { download ->
            if (download.state == Download.STATE_COMPLETED) {
                ...
                val videoDataJson = try {
                    ...
                    Log.e("VideoViewHolder", "Failed to decode JSON string", e)
                    "{}"
                }
                Log.d("DEBUG_JSON", "Raw JSON data: $videoDataJson")

                val videoData = try {
                    AppJson.decodeFromString(VideoData.serializer(), videoDataJson)
                } catch (e: Exception) {
                    Log.e("VideoViewHolder", "Failed to parse VideoData", e)
                    VideoData(
                ...
                val intent = activityProvider?.getOfflinePlayerActivityIntent(videoData, download.request.id)
                    ?: Intent().apply {
                        // Fallback intent if activityProvider is not available
                    }
        ...
        val downloadListener = object : DownloadManager.Listener {
            ...
            override fun onDownloadChanged(
                ...
                handler.post {
                    Log.d("DownloadListener", "Download changed: ${download.request.id} - ${download.state}")
            ...
            override fun onDownloadRemoved(downloadManager: DownloadManager, download: Download) {
                handler.post {
                    Log.d("DownloadListener", "Download removed: ${download.request.id}")
                    val updatedDownloads = VideoDownloadManager.getAllDownloadsWithActiveFirst(context)
                    adapter.updateDownloadsList(updatedDownloads)
        ...
        popupWindow?.setOnDismissListener {
            ...
            VideoDownloadManager.removeDownloadListener(downloadListener)
            popupWindow = null
            Log.d("DownloadPopup", "Popup dismissed")
        }
    ...
    private fun debugDownloadData(download: Download) {
        Log.d("DEBUG_DOWNLOAD", "Download ID: ${download.request.id}")
        Log.d("DEBUG_DOWNLOAD", "Download URI: ${download.request.uri}")
        Log.d("DEBUG_DOWNLOAD", "Data length: ${download.request.data.size}")

        try {
            val dataString = String(download.request.data, Charsets.UTF_8)
            Log.d("DEBUG_DOWNLOAD", "Data content: $dataString")
        } catch (e: Exception) {
            Log.d("DEBUG_DOWNLOAD", "Data is not UTF-8 string")
        }
    }
