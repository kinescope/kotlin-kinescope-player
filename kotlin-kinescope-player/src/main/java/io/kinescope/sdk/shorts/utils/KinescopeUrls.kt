...
class KinescopeUrls(
    ...
    /**
     * Получить список видео через провайдер видео
     * Использует провайдер для получения реальных данных вместо хардкода
     */
    suspend fun getVideosFromApi(): List<VideoData> {
        if (videoProvider == null) {
            Log.w("KinescopeUrls", "VideoProvider is null, using fallback")
            return getNextVideoUrls() // Fallback на хардкод если провайдер не передан
        }

        return try {
            Log.d("KinescopeUrls", "Loading videos from provider. ProjectId: $projectId, FolderId: $folderId")
            val videos = videoProvider.loadVideos(projectId = projectId, folderId = folderId, limit = 50, offset = 0)
            Log.d("KinescopeUrls", "Provider returned ${videos.size} videos")
            if (videos.isEmpty()) {
                Log.w("KinescopeUrls", "Provider returned empty list, using fallback")
            } else {
                Log.d("KinescopeUrls", "Successfully loaded ${videos.size} videos from provider")
                videos.forEachIndexed { index, video ->
                    Log.d(
                        "KinescopeUrls",
                        "Video $index: ${video.title}, HLS: ${video.hlsLink.take(50)}..., DRM license: ${video.drm?.widevine?.licenseUrl?.take(50)}"
                    )
                }
            }
            videos
        } catch (e: Exception) {
            Log.e("KinescopeUrls", "Error loading videos from provider", e)
            Log.e("KinescopeUrls", "Error message: ${e.message}")
            Log.e("KinescopeUrls", "Error stack: ${e.stackTraceToString()}")
    ...
    /**
     * Получить видео по ID через провайдер
     */
    ...
    /**
     * Старый метод с хардкодом (для обратной совместимости)
