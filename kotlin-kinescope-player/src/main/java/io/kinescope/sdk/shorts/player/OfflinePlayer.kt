...
import io.kinescope.sdk.databinding.ActivitySaveVideoPlayerBinding
...
class OfflinePlayer(
    ...
    fun setupPlayer(
        ...
        Log.d("OFFLINE_DEBUG", "=== OFFLINE PLAYER SETUP ===")
        ...
        try {
            val playerView = when (binding) {
                ...
                is ActivitySaveVideoPlayerBinding -> binding.playerView
                    ...
                    // Используем рефлексию для получения playerView из любого binding
                    try {
                        val playerViewField = binding.javaClass.getDeclaredField("playerView")
                        playerViewField.isAccessible = true
                        playerViewField.get(binding) as? PlayerView
                            ?: throw IllegalArgumentException("playerView not found in binding")
                    } catch (e: Exception) {
                        Log.e("OFFLINE_DEBUG", "Failed to get playerView from binding using reflection", e)
            ...
            player.addListener(object : Player.Listener {
                override fun onPlayerError(error: PlaybackException) {
                    Log.e("OFFLINE_ERROR", error.message.toString(), error)
                    player.release()
                    onFallback()
                }
            })
            ...
            Log.e("OFFLINE_DEBUG", "Ошибка при настройке оффлайн-плеера", e)
