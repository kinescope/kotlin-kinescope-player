    ...
    private fun createPlayerListener(position: Int): Player.Listener = object : Player.Listener {
        ...
        override fun onPlaybackStateChanged(playbackState: Int) {
            when (playbackState) {
                ...
                Player.STATE_ENDED -> {
                    //     playerFactory.onPlaybackSuccess()
                    exoPlayer?.seekTo(0)
                    exoPlayer?.playWhenReady = true
        ...
        override fun onPlayerError(error: PlaybackException) {
            Log.e("PLAYER_ERROR", "Error code: ${error.errorCode}, message: ${error.message}", error)
            Log.e("PLAYER_ERROR", "Error cause: ${error.cause?.message}", error.cause)
            ...
            if (isRuntimeError) {
                Log.w("PLAYER_ERROR", "Runtime error detected, attempting recovery...")
                // ����������� ������� �����
                releasePlayer()
                ...
                android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                    try {
                        if (lastVideoData != null && lastBinding != null && lastPosition != -1) {
                            Log.d("PLAYER_ERROR", "Retrying playback...")
                            setupOnlinePlayer(lastVideoData!!, lastPosition, lastBinding!!)
                        }
                    } catch (e: Exception) {
                        Log.e("PLAYER_ERROR", "Failed to retry playback", e)
