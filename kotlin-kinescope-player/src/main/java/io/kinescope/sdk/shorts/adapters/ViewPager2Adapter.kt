...
class ViewPager2Adapter(
    ...
    override fun onBindViewHolder(holder: VideoViewHolder, position: Int) {
        ...
        Log.d("ADAPTER_DEBUG", "Position: $position, Content ID: $contentId")
        Log.d("ADAPTER_DEBUG", "Download found: ${download != null}")
        Log.d("ADAPTER_DEBUG", "Download state: ${download?.state}")
        Log.d("OFFLINE_URI", "URI: ${download?.request?.uri}")

        val isDownloaded = download != null && download.state == Download.STATE_COMPLETED

        if (isDownloaded) {
            Log.d("ADAPTER_DEBUG", "Loading OFFLINE content")
            holder.bindOfflineVideos(videoData, download)
        } else {
            Log.d("ADAPTER_DEBUG", "Loading ONLINE content")
