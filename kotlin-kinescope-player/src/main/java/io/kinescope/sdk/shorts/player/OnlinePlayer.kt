...
class OnlinePlayer(
    ...
    fun setupPlayer(
        ...
        if (videoData.hlsLink.isNullOrBlank()) {
            android.util.Log.e("OnlinePlayer", "HLS link is null or empty")
            return
        }
    ...
    private fun setupDrmPlayer(player: ExoPlayer, videoData: VideoData, binding: ListVideoBinding) {
        if (videoData.hlsLink.isNullOrBlank()) {
            android.util.Log.e("OnlinePlayer", "HLS link is null or empty in setupDrmPlayer")
            return
        }
        
        val licenseUrl = videoData.drm?.widevine?.licenseUrl
        if (licenseUrl.isNullOrBlank()) {
            android.util.Log.e("OnlinePlayer", "DRM license URL is null or empty")
            return
        }
    ...
    private fun setupClearPlayer(player: ExoPlayer, videoData: VideoData, binding: ListVideoBinding) {
        if (videoData.hlsLink.isNullOrBlank()) {
            android.util.Log.e("OnlinePlayer", "HLS link is null or empty in setupClearPlayer")
            return
        }
